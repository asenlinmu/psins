...
void CIMU::SetRFU(const unsigned char *rfu0)
{
...
void IMURFU(CVect3 *pwm, int nSamples, const unsigned char *str)
{
...
void IMURFU(CVect3 *pwm, CVect3 *pvm, int nSamples, const unsigned char *str)
{
...
{
	CSINSTDKF::Init(CSINS(O31,O31,O31));
}
...
/***************************  CAligntrkang  *********************************/
CAligntrkang::CAligntrkang(double vel00, double wz00, double dyaw00):CSINSTDKF(15, 4)
{
...
CAlignsv::CAlignsv(const CVect3 &pos, double ts, double T2, double T1)
{
	pMem = 0;
	Init(pos, ts, T2, T1);
}
...
int CAlignsv::Update(const CVect3 *pwm, const CVect3 *pvm)
{
	...
	*(CVect3*)&wvm[0] = *pwm, *(CVect3*)&wvm[3] = *pvm;
	...
	if(t<T1) {  // align_i0
		...
		if(!alnkfinit) {
			alnkfinit = 1;  tk = 0.0;
			alnkf.Init(CSINS(alni0.qnb0, O31, alni0.pos0));
		}
		for(int i=0; i<2; i++) {
			if(pMem->pop((unsigned char*)wvm)) {
				alnkf.Update((CVect3*)&wvm[0], (CVect3*)&wvm[3], 1, ts);
				tk += ts;  t += ts;
			}
		}
		qnb = alnkf.qnb;
