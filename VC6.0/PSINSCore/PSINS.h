...
class CMathfun {
public:
	friend double r2dm(double r);
	friend double	dm2r(double dm);
	friend BOOL	logtrigger(int n, double f0=1.0);
	friend BOOL	IsZero(double f, double eps=EPS);
	friend int		sign(double val, double eps=EPS);
	friend double	range(double val, double minVal, double maxVal);
	friend double	atan2Ex(double y, double x);
	friend double  diffYaw(double yaw, double yaw0);
	friend double	MKQt(double sR, double tau);
	friend double	randn(double mu, double sigma);
...
//	friend void IMURFU(CVect3 *pwm, int nSamples, const unsigned char *str);
//	friend void IMURFU(CVect3 *pwm, CVect3 *pvm, int nSamples, const unsigned char *str);
};
#else
...
public:
	...
	double dps, dph, dpsh, dphpsh, dphpg, ugpsh, ugpsHz, ugpg2, mpsh, mpspsh, secpsh;
...
public:
	...
	void SetRFU(const unsigned char *rfu0);
...
class CAligntrkang:public CSINSTDKF  // coarse alignment by GNSS velocity track-angle
{
public:
	int cntYawOK;
	...
	CAligntrkang(double vel00=1.0, double wz00=5.0*DPS, double dyaw00=5.0*DEG);
	void Init(const CSINS &sins0=CSINS(O31,O31,O31));
	virtual void SetMeas(void) {};
	int Update(const CVect3 *pwm, const CVect3 *pvm, int nSamples, double ts, const CVect3 &vnr);
};

class CAlignsv  // initial alignment by data save technique
{
public:
...
typedef struct {
	...
	double ptyaw, ptpch, dt, gpsvalid;
} DataSensor;
...
public:
	...
	int load(int lines=1, BOOL txtDelComma=1) {
		...
		pDS->gpsp = LLH(pDS->gpsp.i, pDS->gpsp.j, pDS->gpsp.k);  // data format tranfer
		pDS->ptyaw *= DEG, pDS->ptpch *= DEG;
		pDS->wm *= DEG;
		IMURFU(&pDS->wm, &pDS->vm, 1, "FRD");
		return 1;
	};
};
#endif
...
class CFileIMU6:public CFileRdWt  // CFile Read IMU6
{
public:
