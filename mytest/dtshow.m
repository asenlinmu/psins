function res = dtshow(fres,xyz0,xyz1)
    ...
    if exist('maxlh.txt', 'file')
        ...
        idx = find(res{1}(:,4)>maxlh(2),1);
        if ~isempty(idx)
            plot(res{1}(idx,2)-xyz0(2),res{1}(idx,1)-xyz0(1),'*r','linewidth',3);
        end
        subplot(122), title(sprintf('高程弥散=%.3fm',maxlh(3)));
        idx = find(res{1}(:,4)>maxlh(4),1);
        if ~isempty(idx)
            plot(res{1}(idx,4),res{1}(idx,3)-xyz0(3),'*r','linewidth',3);
        end
    end
