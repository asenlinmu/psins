...
global glv
    ...
    gcp.Cnb = q2cnb(gcp.qnb);
    dvn = gcp.Cnb*dvbm;  % 比力变换
    ...
    % 注意计算顺序！
    gcp.dvn = dvn; gcp.dV = [dVE; dVN; 0];
    gcp.pos = gcp.pos + nTs*[vnr(2)/gcp.eth.RMh;vnr(1)/(gcp.eth.RNh*gcp.eth.cl);vnr(3)]; % 定位更新
    gcp.qnb = qmul( gcp.qnb, rv2q(phim-gcp.Cnb'*(gcp.eth.wnin+gcp.wnc)*nTs) );  % 带反馈控制项wnc的姿态更新
