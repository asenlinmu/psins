...
ns = randn(len,3)*0.001*glv.dpsh*sqrt(ts);
[b, a] = ar1coefs(ts, 10);
wm0 = wm+ns; wm1 = wm+filter(b, a, ns);  % figure,plot([wm0(:,1),wm1(:,1)]);
q1 = qr(1,:)'; q2 = q1; ki = 1;
for k=1:nn:len-nn+1
    k1 = k+nn-1;
	wmi = wm0(k:k1, :);  wmi1 = wm1(k:k1, :);
	q0 = qr(k1+1,:)';
	phim1 = cnscl(wmi, 1);  q1 = qmul(q1,rv2q(phim1)); % optimal method
    phim2 = cnscl(wmi1, 1);  q2 = qmul(q2,rv2q(phim2)); % polynomial method
