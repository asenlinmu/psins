...
glvs
afa = 10*glv.deg;              % half-apex angle
f = 2;  w = 2*pi*f;             % frequency
[nn, ts, nts] = nnts(5, 0.01);  % subsamples & sampling interval
T = 6;                          % simulation time
...
for k=1:nn:len-nn+1
    ...
    phim2 = rvupdt(wmi);  q2 = qmul(q2,rv2q(phim2)); % rvupdt method
