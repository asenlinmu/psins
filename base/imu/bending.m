...
global glv
imu = imuasyn(imu, [0;0;-1.85e-6;0;0;0]);
    if nargin<5, dK=zeros(24,1); end
    if length(dK)==1, dK=zeros(24,1); end  % dK=0
    if length(dK)==24, U=reshape(dK(7:15),3,3)'; W=reshape(dK(16:24),3,3)'; dK=[dK(1:3)';0,dK(4:5)';0,0,dK(6)]; end 
    ts = diff(imu(1:2,end));
    ...
    vm_1=imu(1,4:6)'; dw_1 = zeros(3,1);
    len = length(imu);  phi = [zeros(len,3),imu(:,end)];
    res = [zeros(len,9),imu(:,end)];  afadf=zeros(3,1); afaddw=zeros(3,1);
    timebar(1, len);
    for k=2:len
        wm = imu(k,1:3)'; vm = imu(k,4:6)'; t = imu(k,end);
if wm(2)>0, wm(2)=(1+0.25*glv.ppm)*wm(2); end
if wm(3)>0, wm(3)=(1-2*glv.ppm)*wm(3); end
        df = (vm-vm_1)/ts/ts;  dw = (wm-wm_1)/ts/ts;  ddw = (dw-dw_1)/ts;  dw_1 = dw;
        phim = wm+glv.csCompensate*1/12*cross(wm_1,wm); wm_1 = wm; vm_1 = vm;
        afa=0.95; afadf = afa*afadf + (1-afa)*df; afaddw = afa*afaddw + (1-afa)*ddw;
        phim = phim - dK*wm - (U*afadf+W*afaddw)*ts;
%         phim = phim - dK*wm - (U*df+W*ddw)*ts;
        qnb = qupdt2(qnb, phim, wnie*ts);
        att(k,:) = [q2att(qnb); t]';
        res(k,:) = [afadf; dw; afaddw; t]';
        timebar;
        if t<t1 || t>t2
