function [xk, pk, zk, rk, sk] = xpzrsfile(kfres, q, r, t0)
% The disassemble of KF result array (from C++\CFileRdWt& operator<<(CKalman &kf)).
%
% Prototype: [xk, pk, zk, rk, sk] = xpzrsfile(kfres, q, r, t0)
...
% Example:
%     psinstypedef(196);
%     [xk,pk,zk,rk,sk] = xpzrsfile('kf.bin', 28,6,t0);
%     kfplot(xk,pk,1:19);
...
% 23/12/2020
    ...
    switch nargout
        case 1,  % xkpk = xpzrs(kfres, q, r)
        xk = kfres(:,[1:2*q,end]);
        case 2,  % [xkpk, zrk] = xpzrs(kfres, q, r)
        xk = kfres(:,[1:2*q,end]);
        pk = kfres(:,[2*q+1:2*q+2*r,end]);
        case 3,  % [xkpk, zrk, sk] = xpzrs(kfres, q, r)
        xk = kfres(:,[1:2*q,end]);
        pk = kfres(:,[2*q+1:2*q+2*r,end]);
        zk = kfres(:,end-1:end);
        case 5,  % [xk, pk, zk, rk, sk] = xpzrs(kfres, q, r)
        xk = kfres(:,[1:q,end]);
